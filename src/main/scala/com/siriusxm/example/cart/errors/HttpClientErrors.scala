...
object HttpClientErrors {
  ...
  final case class ClientError(m: String) extends HttpError(m)
  ...
  object ErrorHandlers {
    def httpClientErrorResponseHandler[F[_]: Async, T](
      name: String,
      response: Response[F]
      ...
      case r =>
        r.status match {
          ...
          case _ =>
            Async[F].pure(
              Left[ApplicationError, T](ClientError(s"Request failed with status ${r.status.code}"))
            )
        }
    }
  }
