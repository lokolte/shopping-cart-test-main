...
package object models {
  ...
  case class Cart(items: Set[Item]) {
    private def updateSet(items: Set[Item], item: Item): Set[Item] =
      items - item +
        items.find(_ == item)
          .map(newItem =>
    ...
    def addItem(item: Item): Cart =
      Cart(items = updateSet(items, item))

    def addItems(newItems: Set[Item]): Cart =
      Cart(items = newItems.foldLeft(items)((updates, item) => updateSet(updates, item)))
